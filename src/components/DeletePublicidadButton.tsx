...
import { deletePublicidad } from "@/app/actions/publicidad";

interface DeletePublicidadButtonProps {
  publicidadId: number;
  publicidadLink: string;
}

export function DeletePublicidadButton({ publicidadId, publicidadLink }: DeletePublicidadButtonProps) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const result = await deletePublicidad(publicidadId);
      if (result.error) {
  ...
  return (
    <>
      ...
      <button
        ...
        className="text-destructive hover:text-destructive/80 transition-colors p-1.5 rounded-sm hover:bg-destructive/10"
        ...
        >
          ...
          <div className="p-6 space-y-4 text-left">
            <p className="text-sm text-muted-foreground">
              ¿Estás seguro de eliminar este anuncio publicitario (
              <span className="font-semibold text-foreground break-all">{publicidadLink}</span>)? Esta acción no se puede deshacer.
            </p>
