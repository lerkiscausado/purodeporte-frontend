...
import { GestionarTorneoClient } from "./GestionarTorneoClient";
...
export default async function GestionarTorneoPage({ searchParams }: PageProps) {
  ...
  if (token) {
    try {
      // 1. Fetch de los detalles del Torneo
      ...
      // 2. Fetch de los partidos del Torneo
      if (torneo) {
        const partidosResponse = await fetch(getApiUrl(`partidos/torneo/${id}`), {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          next: { revalidate: 0 },
        ...
        if (partidosResponse.ok) {
          partidos = await partidosResponse.json();
        }
      }
    } catch (error) {
      console.error("Error al cargar datos de gestión de torneo:", error);
